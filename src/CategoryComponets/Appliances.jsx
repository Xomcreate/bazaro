...
function Appliances() {
  ...
  const BrandFilter = ({ brands }) => (
    <div className="p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
      <h3 className="text-xl font-bold mb-4 text-black">Brand</h3>
      <div className="space-y-3 h-96 overflow-y-auto pr-2">
        {brands.map((brand) => (
          <label key={brand} className="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition">
            ...
            <span className="ml-3 text-sm font-medium text-gray-700">{brand} <span className="text-xs text-gray-400">({applianceProducts.filter(p => p.brand === brand).length})</span></span>
          </label>
        ))}
      </div>
    </div>
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <main className="w-full lg:w-3/4">
            <h2 className="text-2xl font-bold text-black mb-4 border-b pb-2">Featured Appliances</h2>
